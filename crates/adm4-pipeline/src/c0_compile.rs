...
use adm4_decision::{DesignLevel, ParameterSchema, ParameterValues, Selection};
...
pub fn compile_frozen_design(frozen: &FrozenDesign, space: &DesignSpace) -> Adm4Result<GameSpec> {
    ...
    for selection in &frozen.decisions {
        ...
        let Some(option) = point.option(&selection.option_id) else {
            unknown.push(UnclassifiedItem {
                item: format!("{}/{}", selection.decision_id, selection.option_id),
                reason: "选项不存在".into(),
            });
            continue;
        };
        ...
        match role.as_str() {
            "title" => {
                if let Some(title) = scalar_text(&selection.parameters, "title") {
                    intent.title = title;
                }
                intent
                    .profile
                    .insert(selection.decision_id.clone(), option.label.clone());
                source_map.push(entry("intent", &selection.decision_id));
            }
            "profile" => {
                intent.profile.insert(
                    selection.decision_id.clone(),
                    format!(
                        "{}{}",
                        option.label,
                        render_scalar_suffix(&selection.parameters)
                    ),
                );
                source_map.push(entry("intent", &selection.decision_id));
            }
            "promise" => {
                promises.push(format!("{}：{}", option.label, selection.rationale));
                source_map.push(entry("intent", &selection.decision_id));
            }
            "genre" => {
                genre_parts.push(option.label.clone());
                source_map.push(entry("intent", &selection.decision_id));
            }
            "system" => {
                systems.push(SystemSpec {
                    id: selection.decision_id.clone(),
                    name: option.label.clone(),
                    purpose: option.summary.clone(),
                    interfaces: option.implications.clone(),
                });
                source_map.push(entry(
                    &format!("systems/{}", selection.decision_id),
                    &selection.decision_id,
                ));
            }
            "mechanic" => match compile_mechanic(space, selection, point, option) {
                Ok(mechanic) => {
                    source_map.push(entry(
            ...
            "entity_table" => match compile_entity_table(selection, option) {
            ...
            "data_table" => match compile_data_table(selection, option) {
                Ok(table) => {
                    source_map.push(entry(
                        &format!("tables/{}", table.id),
                        &selection.decision_id,
            ...
            "content" => {
                let data = serde_json::to_value(&selection.parameters).map_err(|error| {
                ...
                source_map.push(entry(
                    &format!("content/{}", selection.decision_id),
                    &selection.decision_id,
                ));
            }
            other => {
...
fn compile_mechanic(
    ...
    let system_id = option
        ...
        .or_else(|| {
            space
                ...
                .iter()
                .find(|candidate| {
                    candidate.domain == point.domain && candidate.level == DesignLevel::L3
                })
                .map(|candidate| candidate.id.clone())
        })
        .ok_or_else(|| UnclassifiedItem {
            item: selection.decision_id.clone(),
            reason: "机制无法归属到任何系统（无 system 标签且同域无 L3 决策）".into(),
        })?;
    ...
    for template in &option.effects_template {
        let substituted = substitute_placeholders(template, &selection.parameters);
...
fn compile_entity_table(
    ...
    for row in rows {
        ...
        entities.push(EntitySpec {
            id: format!("{}.{row_id}", selection.decision_id),
            name: row_id,
