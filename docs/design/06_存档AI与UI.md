...
> **实现对齐说明（2026-08-29）**：当前桌面页签为 项目 | 设计工作台 | AI 访谈 | 冻结门 |
> 开发流水线 | 模板逆向 | 运行日志。「打包」「SDK 知识库」本期未实现（随 Phase 2 立项再议）；
> AI 访谈独立成签。
```
...
> `ai secret-set|provider-set|invoke-check` 细分、`deliver package|doctor` 本期未实现。
